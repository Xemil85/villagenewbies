    ...
    private async void OnMuodostaLaskuClicked(object sender, EventArgs e)
    {
        if (sender is Button button && button.CommandParameter is int varausId)
        {
            ...
            if (varaus != null)
            {
                ...
                // Tarkista, onko lasku jo luotu t‰lle varaukselle
                bool onkoLaskuLuotu = await databaseAccess.OnkoLaskuLuotu(varausId);
                if (onkoLaskuLuotu)
                {
                    await DisplayAlert("Lasku on jo olemassa", "T‰lle varaukselle on jo muodostettu lasku.", "OK");
                    return; // Keskeyt‰, jos lasku on jo olemassa
